...
export namespace Iterators {
   ...
   */
  export function asIterator<T>(elements: readonly T[]): IterableIterator<T> {
   ...
   */
  export function* cycle<T>(elements: readonly T[], start?: T): IterableIterator<T> {
...
export class TopDownTreeIterator extends AbstractTreeIterator {
  protected iterator(root: TreeNode): IterableIterator<TreeNode> {
    ...
    return (function* (): IterableIterator<TreeNode> {
      let maybeNext: TreeNode | undefined = root;
      let next = maybeNext;
      while (maybeNext) {
        next = maybeNext;
        yield next;
        maybeNext = doNext(next);
      }
    })();
  }
